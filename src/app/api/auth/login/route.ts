...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(joinUrl(baseUrl, "/auth/login"), {
    method: "POST",
  ...
  // Upstream returns { access_token, user }. We store token in httpOnly cookie and
  // don't expose it to browser JS.
